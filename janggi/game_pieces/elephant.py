...
class Elephant(GamePiece):
    """
    Represents the 'Elephant' game piece.
    """
    ...
    def get_valid_moves(self, board):
        """
        Returns a list of all valid moves the Elephant GamePiece can make from its current position. Specifically, positions will be 
        ...
        for i in range(-3, 4, 6):                   # length of move
            for j in range(-2, 3, 4):               # width of move
                for k in range(2):                  
                    if k % 2 == 0:                  # flips reference to rows and cols to calculate move at mirrorred location
                        x, y = i, j
                    else: x, y = j, i
                    ...
                    if col + x in board.board_ranges()[0] and row + y in board.board_ranges()[1]:
                        if x == i:
                            if i < 0:
                                potential_block1 = board.get_space_contents((col + (i // 2) + 1, row))
                            else:
                                potential_block1 = board.get_space_contents((col + (i // 2), row))
                        else:
                            if i < 0:
                                potential_block1 = board.get_space_contents((col, row + (i // 2) + 1))
                            else:
                                potential_block1 = board.get_space_contents((col, row + (i // 2)))
                        if potential_block1 == '---':
                            if x == i:
                                if i < 0:
                                    potential_block2 = board.get_space_contents((col + (i // 2), row + (j // 2)))
                                else:
                                    potential_block2 = board.get_space_contents((col + (i // 2) + 1, row + (j // 2)))
                            else:
                                if i < 0:
                                    potential_block2 = board.get_space_contents((col + (j // 2), row + (i // 2)))
                                else:
                                    potential_block2 = board.get_space_contents((col + (j // 2), row + (i // 2) + 1))
                            if potential_block2 == '---':
